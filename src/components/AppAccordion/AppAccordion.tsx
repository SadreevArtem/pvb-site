...
export const AppAccordion: React.FC<Props> = ({
  ...
  return (
    <div className={clsx(" bg-white md:ml-[160px]", className)}>
      <button
        className="flex justify-between items-center w-full text-start gap-4 p-4 "
        aria-expanded={open}
        {...(open ? { "aria-controls": controlId } : {})}
        onClick={() => setOpen(!open)}
      >
        <span
          className={clsx(" title font-normal hover:underline", {
